...
class Page_sistemaController extends Page_mainController
{
	...
	public function indexAction()
	{
		...
		if(count($existe)==0 and $_SESSION['kt_login_level']=="2"){
		...
		if($usuariosinfo->fecha_nacimiento!=""){
			$this->_view->fecha_nacimiento = $usuariosinfo->fecha_nacimiento;
		}
		if($usuariosinfo->fecha_ingreso!=""){
			$this->_view->fecha_ingreso = $usuariosinfo->fecha_ingreso;
		}
		if($usuariosinfo->fecha_documento!=""){
			$this->_view->fecha_documento = $usuariosinfo->fecha_documento;
		}
		...
		if($usuariosinfo->estado_civil!=""){
			$this->_view->estado_civil = $usuariosinfo->estado_civil;
		}
	...
	public function paso8Action()
	{
		...
		$tabla = '<img src="http://fendesacreditos.omegasolucionesweb.com/corte/banner.png"><br>'.$tabla;
	...
	public function asignaranalistaAction(){
        ...
        if($solicitud->tipo_garantia=="2"){
			...
			$content = "<img src='http://fendesacreditos.omegasolucionesweb.com/corte/banner.png'><br>Estimado(a) <b>".$nombres_codeudor."</b>, el/la asociado(a) <b>".$nombres_asociado."</b> lo ha ingresado como codeudor en la solicitud de crédito <b>No. WEB".$numero."</b> del sistema de solicitudes de crédito de FESDIS por valor de <b>$".number_format($valor)."<b/><br><br>Por favor ingrese en el siguiente enlace para completar su información personal: <a href='https://creditosfondtodos.com.co/page/codeudor/?id=".$id."&e=".$correo_codificado."'><button type='button' class='btn btn-primary' style='background:#0084C9; color:#FFFFFF; padding:5px 10px;'>Ingresar</button></a>";
	...
	public function filtrolineaAction()
	{
		...
		$ahorrosModel = new Page_Model_DbTable_Ahorros();
		$ahorros_list = $ahorrosModel->getList(" estadocuenta_ahorros_modalidad='AHORRO VOLUNTARIO' AND estadocuenta_ahorros_cedula='$cedula' ","");
		$ahorros = 0;
		foreach ($ahorros_list as $key => $value) {
			$ahorros = $ahorros + $value->estadocuenta_ahorros_saldo;
		}

		$aportesModel = new Page_Model_DbTable_Aportes();
		$aportes_list = $aportesModel->getList(" cedula='$cedula' ","");
		$aportes = $aportes_list[0]->total_aportes;


		$smlv = 877803;
		//$aportes = 0; //aportes sociales y ahorro permantente
		//$ahorros = 0; //ahorro voluntario
		...
		if($linea=="CE"){
			$saldos_list = $saldosModel->getList(" estadocuenta_saldos_linea = 'EDUCACION DE 25 A 36 MESES' AND estadocuenta_saldos_cedula='$cedula' ","");
			$saldos=0;
			foreach ($saldos_list as $key => $value) {
				$saldos+=$value->estadocuenta_saldos_stotal;
			}
			$cupo_actual = $smlv*20 - $saldos;
		...
		if($linea=="CVV"){
			$saldos_list = $saldosModel->getList(" estadocuenta_saldos_linea = 'VIVIENDA  60 MESES' AND estadocuenta_saldos_cedula='$cedula' ","");
			$saldos=0;
			foreach ($saldos_list as $key => $value) {
				$saldos+=$value->estadocuenta_saldos_stotal;
			}
			$cupo_actual = (20*$aportes) + (5*$ahorros) - $saldos;
			$max = $smlv*250;
			if($cupo_actual > $max){
				$cupo_actual = $max;
			}
		...
		if($linea=="CVH"){
			$saldos_list = $saldosModel->getList(" estadocuenta_saldos_linea = 'VEHICULO 60 MESES' AND estadocuenta_saldos_cedula='$cedula' ","");
			$saldos=0;
			foreach ($saldos_list as $key => $value) {
				$saldos+=$value->estadocuenta_saldos_stotal;
			}
			$cupo_actual = (10*$aportes) + (3*$ahorros) - $saldos;
			$max = $smlv*150;
			if($cupo_actual > $max){
				$cupo_actual = $max;
			}
		...
		if($linea=="CM"){
			$saldos_list = $saldosModel->getList(" estadocuenta_saldos_linea = 'MULTIPROPOSITO 37 A 48 MESES' AND estadocuenta_saldos_cedula='$cedula' ","");
			$saldos=0;
			foreach ($saldos_list as $key => $value) {
				$saldos+=$value->estadocuenta_saldos_stotal;
			}
			$cupo_actual = (7*$aportes) + (2*$ahorros) - $saldos;
			$max = $smlv*70;
			if($cupo_actual > $max){
				$cupo_actual = $max;
			}
		...
		if($linea=="CF"){
			$saldos_list = $saldosModel->getList(" estadocuenta_saldos_linea = 'CREDIFACIL 7 A 12 MESES' AND estadocuenta_saldos_cedula='$cedula' ","");
			$saldos=0;
			foreach ($saldos_list as $key => $value) {
				$saldos+=$value->estadocuenta_saldos_stotal;
			}
			$cupo_actual = (6*$aportes) + (2*$ahorros) - $saldos;
			$max = $smlv*4;
			if($cupo_actual > $max){
				$cupo_actual = $max;
			}
		...
		if($linea=="CCC"){
			$saldos_list = $saldosModel->getList(" estadocuenta_saldos_linea = 'COMPRA CARTERA 37 A 48 MESES' AND estadocuenta_saldos_cedula='$cedula' ","");
			$saldos=0;
			foreach ($saldos_list as $key => $value) {
				$saldos+=$value->estadocuenta_saldos_stotal;
			}
			$cupo_actual = (8*$aportes) + (3*$ahorros) - $saldos;
			$max = $smlv*70;
			if($cupo_actual > $max){
				$cupo_actual = $max;
			}
	...
	public function recogerAction()
	{
		...
		foreach ($saldos as $key => $value) {
			$tabla .= '
					<tr>
						<td><a class="enlace_modal" data-toggle="modal" data-target="#modal_recoger'.$key.'">'.$value->estadocuenta_saldos_numero.'</a></td>
						<td>$'.number_format($value->estadocuenta_saldos_vinicial).'</td>
						<td>$'.number_format($value->estadocuenta_saldos_stotal).'</td>
						<td align="center"><input type="checkbox" id="saldo'.$key.'" onclick="sumar_saldos('.$key.')" /></td>
					</tr>
					<input type="hidden" value="'.$value->estadocuenta_saldos_stotal.'" id="valor_saldo'.$key.'" />
					<input type="hidden" value="'.$value->estadocuenta_saldos_numero.'" id="numero'.$key.'" />
			';
		...
		if($solicitud->recoger_credito=="1"){
			$tabla.='
			  ...
			  <tr>
			    ...
			    <td align="right">$'.$this->formato_pesos($saldo).'</td>
			  </tr>';
		}
