...
class Page_sistemaController extends Page_mainController
{
	...
	public function indexAction()
	{
		...
		$this->_view->lineas = $lineaModel->getList(" activo='1' "," nombre ASC ");
		...
		$filtro_nomina=" AND lineas_credito.codigo!='662' AND lineas_credito.codigo!='455' AND lineas_credito.codigo!='29A' AND lineas_credito.codigo!='26A' "; //quitar calamidad domestica y encuentros deportivos
		if($nomina_list->codigo_nomina=="101"){ //bbva colombia
			$filtro_nomina=" ";
		}
		if($nomina_list->codigo_nomina=="118" or $nomina_list->codigo_nomina=="103" or $nomina_list->codigo_nomina=="104" or $nomina_list->codigo_nomina=="116" or $nomina_list->codigo_nomina=="117" or $nomina_list->codigo_nomina=="120"){ //comercializadora financiera
			/*
			103-FIDUCIARIA BBVA
			104-BBVA VALORES
			116-BBVA SEGUROS VIDA
			117-BBVA SEGUROS GENERALES
			120-OPPLUS OPERACIONES Y SERVICIOS
			*/
			$filtro_nomina=" AND lineas_credito.codigo!='662' AND lineas_credito.codigo!='29A' ";
		}
		...
		$cupo_actual = $cupos_list[0]->cupo;
		$salario = $nomina_list[0]->cupo;

		if($linea_id==28 or $linea_id==44){ //PAGA PRIMA
			if($nomina_list[0]->codigo_nomina=="101"){
				$cupo_actual = round($salario*80/100);
			}else{
				$cupo_actual = round(($salario/2)*80/100);
			}
		}

		if($linea_id==25 or $linea_id==42){ //FOE CONSUMO
			$cupo_actual = round($salario*5);
			if($cupo_actual>12000000){
				$cupo_actual=12000000;
			}
		}

		$saldo_actual = $cupos_list[0]->saldo_actual;
		...
		if($valor1==""){
			$valor1 = formato_pesos($valor_min);
		}
		...
		//agrupar o no segun la linea
		/*
		·         --152 - LIBRE INVERSION
		·         441 - FOE EDUCACION
		·         445 - FOE CONSUMO
		·         446 - FOE EXPRESS
		·         448 - FERIA DE LA FAMILIA FOE
		·         454 - TURISMO
		*/
		$monto_aux = $monto_solicitado;
		if($linea_id==19 or $linea_id==25 or $linea_id==18 or $linea_id==24 or $linea_id==26 or $linea_id==41 or $linea_id==42 or $linea_id==43 or $linea_id==45 or $linea_id==47){
			$monto_aux = $valor;
		}
		...
		$i = $tasa/100;
		$seguro = 0.02/100;
		$k1 = $monto_aux - $presente; // prestamo
		$n=$max_meses;
		if($cuotas!=""){
			$n = $cuotas; //cuotas
		}
	...
	public function paso1Action()
	{
		...
		$ultima = $solicitudModel->getList(" cedula='$cedula' AND paso='7' "," id DESC ")[0];
	...
	public function guardarpasoAction()
	{
		...
		if($id>0){
			...
			if($paso=="5"){
				...
				if($solicitud->linea!="44" and $edad<75){ //pagaprima
	...
	public function solicitudesAction()
	{
		...
		foreach ($lineas as $key => $value) {
			$lineas_array[$value->id] = $value->nombre;
		}
