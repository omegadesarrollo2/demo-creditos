...
class Page_sistemaController extends Page_mainController
{
	...
	public function indexAction()
	{
		...
		if($fecha_min_2m<$fecha_afiliacion){
			$filtro_lineas .=" AND codigo!='TR' AND codigo!='LI' AND codigo!='FE' AND codigo!='AP' AND codigo!='SA' AND codigo!='EDU' AND codigo!='CD'";
		}
		if($fecha_min_6m<$fecha_afiliacion){
			$filtro_lineas .=" AND codigo!='CC'";
		}
		if($fecha_min_3m<$fecha_afiliacion){
			$filtro_lineas .=" AND codigo!='CCV'";
		}
	...
	public function paso8Action()
	{
		...
		$tabla = '<img src="http://creditos.fesdis.com.co:8081/corte/banner.png"><br>'.$tabla;
	...
	public function resumenAction()
	{
		...
		$tabla = $this->generartabla2($numero,$usuario,$solicitud,$lineas,$analista);
	...
	public function asignaranalistaAction(){
		...
		$analistas = $usuariosModel->getList(" user_level = '3' AND user_state='1' AND user_user='Maria' ","");
		...
		if($solicitud->paso=='8' and $solicitud->asignado>0){
			...
			$content.="<br><br>
			<div align='center'>
				<a href='http://creditos.fesdis.com.co:8081/administracion/solicitudes/detalle/?id=".$id."'><button type='button' class='btn btn-primary' style='background:#0084C9; color:#FFFFFF; padding:5px 10px;margin-right:20px; text-decoration:none;'>Detalle solicitud</button></a>
				<a href='http://creditos.fesdis.com.co:8081/administracion/solicitudes/manage/?id=".$id."'><button type='button' class='btn btn-primary' style='background:#0084C9; color:#FFFFFF; padding:5px 10px; text-decoration:none;'>Editar solicitud</button></a>
			</div>
			";
	        ...
	        if($solicitud->tipo_garantia=="2"){
				...
				$content = "<img src='http://creditos.fesdis.com.co:8081/corte/banner.png'><br>Estimado(a) <b>".$nombres_codeudor."</b>, el/la asociado(a) <b>".$nombres_asociado."</b> lo ha ingresado como codeudor en la solicitud de crédito <b>No. WEB".$numero."</b> del sistema de solicitudes de crédito de FESDIS por valor de <b>$".number_format($valor)."<b/><br><br>Por favor ingrese en el siguiente enlace para completar su información personal: <a href='http://creditos.fesdis.com.co:8081/page/codeudor/?id=".$id."&e=".$correo_codificado."&n=1'><button type='button' class='btn btn-primary' style='background:#0084C9; color:#FFFFFF; padding:5px 10px;'>Ingresar</button></a>";
				...
				if($codeudor->id>0){
					...
					$content = "<img src='http://creditos.fesdis.com.co:8081/corte/banner.png'><br>Estimado(a) <b>".$nombres_codeudor."</b>, el/la asociado(a) <b>".$nombres_asociado."</b> lo ha ingresado como codeudor en la solicitud de crédito <b>No. WEB".$numero."</b> del sistema de solicitudes de crédito de FESDIS por valor de <b>$".number_format($valor)."<b/><br><br>Por favor ingrese en el siguiente enlace para completar su información personal: <a href='http://creditos.fesdis.com.co:8081/page/codeudor/?id=".$id."&e=".$correo_codificado."&n=2'><button type='button' class='btn btn-primary' style='background:#0084C9; color:#FFFFFF; padding:5px 10px;'>Ingresar</button></a>";
	...
	public function filtrolineaAction()
	{
		...
		if($linea=="ING"){
			$cupo_actual=$cupo_max= 3000000;
			$saldos_list = $saldosModel->getList("estadocuenta_saldos_linea = '11' AND estadocuenta_saldos_cedula='$cedula' ","");
			$saldos=0;
			foreach ($saldos_list as $key => $value) {
		...
		if($linea=="TR"){
			$cupo_actual=$cupo_max= 900000;
			$saldos_list = $saldosModel->getList("estadocuenta_saldos_linea = 6 AND estadocuenta_saldos_cedula='$cedula' ","");
			$saldos=0;
			foreach ($saldos_list as $key => $value) {
				$saldos=$saldos+$value->saldoactual;
			}
			$cupo_actual = $salario*0.60;
			
			if($cupo_actual > $cupo_max){
				$cupo_actual = $cupo_max;
			}
			$cupo_actual = $cupo_actual - $saldos;
			//$respuesta['saldos']="estadocuenta_saldos_linea = 6 AND estadocuenta_saldos_cedula='$cedula'";
		}
		if($linea=="LI"){
			$cupo_actual=$cupo_max= $smlv*30;
			$cupo_max2=$aportes*5;
			$cupo_max3=$ahorros*5;
		...
		if($linea=="FE"){
			$cupo_actual=$cupo_max= 5000000;
			$saldos_list = $saldosModel->getList(" estadocuenta_saldos_linea LIKE 4 AND estadocuenta_saldos_cedula='$cedula' ","");
			$saldos=0;
			foreach ($saldos_list as $key => $value) {
				$saldos=$saldos+$value->saldoactual;
			}
		...
		if($linea=="AP"){
			$cupo_actual=$cupo_max=  $salario*0.90;;
		...
		if($linea=="EDU"){
			$cupo_actual=$cupo_max= $smlv*10;
			$cupo_max2=$aportes*5;
			$cupo_max3=$ahorros*5;
		$saldos_list = $saldosModel->getList(" estadocuenta_saldos_linea = 9 AND estadocuenta_saldos_cedula='$cedula' ","");
			$saldos=0;
			foreach ($saldos_list as $key => $value) {
				$saldos=$saldos+$value->saldoactual;
		...
		if($linea=="CC"){
			$cupo_actual=$cupo_max= $smlv*60;
			$saldos_list = $saldosModel->getList("estadocuenta_saldos_linea = 5 AND estadocuenta_saldos_cedula='$cedula' ","");
			$saldos=0;
			foreach ($saldos_list as $key => $value) {
				$saldos=$saldos+$value->saldoactual;
			}
		...
		if($linea=="CCV"){
			$cupo_actual=$cupo_max= $smlv*10;
			$cupo_max2=$aportes*5;
			$cupo_max3=$ahorrototal*5;
			$saldos_list = $saldosModel->getList(" estadocuenta_saldos_linea = 10 AND estadocuenta_saldos_cedula='$cedula' ","");
			$saldos=0;
			foreach ($saldos_list as $key => $value) {
				$saldos=$saldos+$value->saldoactual;
			}
		...
		if($linea=="VEH"){
			$cupo_actual=$cupo_max= $smlv*40;

			$saldos_list = $saldosModel->getList(" estadocuenta_saldos_linea = 8 AND estadocuenta_saldos_cedula='$cedula' ","");
			$saldos=0;
			foreach ($saldos_list as $key => $value) {
				$saldos=$saldos+$value->saldoactual;
			}
		...
		if($linea=="CV"){
			$cupo_actual=$cupo_max= $smlv*62;
	
			$saldos_list = $saldosModel->getList(" estadocuenta_saldos_linea = 12 AND estadocuenta_saldos_cedula='$cedula' ","");
			$saldos=0;
			foreach ($saldos_list as $key => $value) {
				$saldos=$saldos+$value->saldoactual;
			}
		...
		// rango cuotas
			if($linea=="LI"){
			$rango=$rangosModel->getList("rango_codigo=3","");
	...
	public function recogerAction()
	{
		...
		$saldosModel = new Page_Model_DbTable_Saldos();
		$saldos = $saldosModel->getList(" estadocuenta_saldos_cedula='$cedula' ","");
	...
	function generartabla2($numero,$usuario,$solicitud,$lineas,$analista){

		$nombres = $solicitud->nombres." ".$solicitud->nombres2." ".$solicitud->apellido1." ".$solicitud->apellido2;
		$garantias = array("","APORTES SOCIALES INDIVIDUALES","DEUDOR SOLIDARIO","AFIANZADORA","HIPOTECARIA","PRENDARIA");

		$tabla .= '
		<div class="fondo-verde p-2 text-center mb-4">
