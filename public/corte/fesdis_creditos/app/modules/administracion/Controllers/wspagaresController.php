...
class Administracion_wspagaresController extends Administracion_mainController
{
	...
	public function crearpagareAction()
	{
		...
		if(count($existe_pagare)==0){
			...
			$id_clase_documento = 598; //id plantilla
			//produccion
			      ...
			      <ser:creacionPagaresCodificado>
			         <arg0>
			            <documentoPagareServiceDTO>
							...
							<valorPesosDesembolso>'.$monto.'</valorPesosDesembolso>
							<ciudadDesembolso>'.$ciudad.'</ciudadDesembolso>
							<departamento>'.$departamento.'</departamento>
							<pais>'.$pais.'</pais>
							<tasaInteres>'.$tasa_interes.'</tasaInteres>
	...
	private function enviarestado($id){
        ...
        $emailModel->getMail()->setFrom("notificaciones@fonkoba.com", "Notificaciones Sistema Solicitud de créditos");
		...
		if($correo_gestor!=""){
			$emailModel->getMail()->addCC("".$correo_gestor);
		}
		if($correo_analista!=""){
			$emailModel->getMail()->addCC("".$correo_analista);
		}
